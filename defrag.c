...
void defrag () {
	// TODO: MAX arr len handle
	int max_len_temp = 15;

	int free_index_arr[max_len_temp], free_index_arr_len;
	index_arr_free_list(free_index_arr, &free_index_arr_len);

	// temporary for 1 list
	int list_index_arr[max_len_temp], list_index_arr_len;
	index_arr_list(0, list_index_arr, &list_index_arr_len);
...
//		printf("\n%d %d", free_index_arr[m], combined_arr[m]);
		if (free_index_arr[m] != combined_arr[m]) {
			int fl_index = free_index_arr[m];
			int cl_index = combined_arr[m];
			exchange_nodes(fl_index, cl_index, &free_index, &lists_index_no[0]);
		}
	}
}
